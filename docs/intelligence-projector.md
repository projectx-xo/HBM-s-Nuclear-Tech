...
The updated right-click panel filters one combined snapshot into **Surface**, **Subsurface**, and **Combined** layers. Surface retains blocks at local ground level and above; Subsurface retains blocks below ground; Combined retains both at their original coordinates. Ground is estimated from exposed natural blocks in the captured scene and extended beneath buildings from the nearest exposed column. Leaves and plants do not define ground. If the entire footprint is built over, the highest captured natural ground is the fallback; this estimate may be imperfect under extensive construction or unusual overhangs.

Layer selection resets floor and section cuts, hides terrain, and selects all findings in that layer. **Floor** clips the ceiling without changing the layer; its first decrement starts at the highest visible block. **All floors** restores full height. **Section** cycles off, X, Z, with a separate position control for revealing rooms and shafts. Findings are filtered by their coordinate bounds and keep their original scan numbers; a finding crossing ground level can appear in both layers. These are visualization filters, not separate scan sources.

The native OC API accepts `configure("view", "surface")`, `configure("view", "subsurface")`, and `configure("view", "combined")`. Existing scripts using exterior/interior/cutaway remain compatible as clipping presets within Combined. The existing CENTRAL commands below use those legacy presets:
...
- Registry names and four-bit metadata are saved in a compact palette, so textures do not depend on matching numerical block IDs. Vanilla block shapes use Minecraft's block renderer with captured neighbors. Silo hatches (both sizes), standard/large launch pads and custom-missile launch tables use their original models with captured hatch opening, launcher pose and loaded missile appearance. These are frozen visual snapshots: they do not run machinery or consult the live base. A fresh scan is required after installing this support; older snapshots retain their previous appearance. Other custom blocks still use textured captured bounds, approximated at half-block resolution; ordinary full-cube HBM blocks retain their exact textures. Biome-specific tint and flying entities are not captured. Finding symbols remain available alongside the models.
...
- Machine capture is capped at 256 models per snapshot; excess machines retain block geometry. Dummy chains are bounded and never followed outside the loaded scan footprint. Only visual fields and item registry names are stored, not inventories or launch/control data. Floor and section cuts clip the models; Surface/Subsurface use the estimated ground at each machine core.
...
- Fully enclosed blocks are skipped. The client compiles opaque and transparent block geometry in bounded batches and caches it. Rotation, scale and finding selection reuse the cache; layer changes, cuts, terrain changes and texture reloads rebuild it. A 65,536-visible-block limit reports truncation on the table; a tighter floor/side cut reduces complexity.
...
Machine snapshot checks cover pose persistence, visual-only data, malformed model data and legacy snapshots. An isolated client fixture captured open/closed large hatches, standard and large missile pads, and a custom-missile launch table. Capture ran with OpenComputers installed; a separate client run without OpenComputers visually verified the models and floor clipping after injecting the saved snapshot into the local table. This fixture does not establish satellite transfer or dedicated-server behavior.
